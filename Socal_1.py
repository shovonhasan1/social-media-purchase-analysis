...
# 2. File paths
DATA_FILE   = Path(r"C:\Users\user\PycharmProjects\Social Media\Social_Media_Dataset.xlsx")
OUTPUT_FILE = Path(r"C:\Users\user\PycharmProjects\Social Media\Impulse_Radar_Output.xlsx")
...
# 4A. Deduplicate: drop rows identical across *all* columns
df = df.drop_duplicates()

# 5. Drop rows without target label
df = df.dropna(subset=[TARGET])

# 6. Explode multi-platform users ("IG;TikTok" → 2 rows)
...
# 8. OneHotEncoder – handles both old & new scikit-learn versions
try:
